...
///////////////////////////////////////////
    ...
    public void Init() {
        ...
        Sound.getInstance().backgroundPlay(R.raw.insect_voice);
...
*/
    ...
    public void NetWorkLoadingMap()
    {
        MyListImmforList= St_Network.listItems;
        AnaMap=new Map_analysis(MyListImmforList.get(0).getData(6));
        Unit temp;
        for(int i=0;i<AnaMap.ArrMap.size();i++)
        {
            switch (AnaMap.ArrMap.get(i).GetPosition())
            {
                case f_tower:
                    m_bmap[AnaMap.ArrMap.get(i).GetX()][AnaMap.ArrMap.get(i).GetY()] = f_tower;
                    temp = new Unit(AppManager.getInstance().getBitmap(R.drawable.archortower));
                    temp.SetPos(AnaMap.ArrMap.get(i).GetX(), AnaMap.ArrMap.get(i).GetY());
                    temp.resizebitmap(100 - 100 / 3, 60);
                  PlayerUnit.add(new UnitManager(temp,30,1,f_tower));
                    break;
                case f_jumpingtrap:
                    m_bmap[AnaMap.ArrMap.get(i).GetX()][AnaMap.ArrMap.get(i).GetY()] = f_jumpingtrap;

                    temp = new Unit(AppManager.getInstance().getBitmap(R.drawable.trap));
                    temp.SetPos(AnaMap.ArrMap.get(i).GetX(), AnaMap.ArrMap.get(i).GetY());
                    temp.resizebitmap(25, 25);
                     PlayerUnit.add(new UnitManager(temp,0,1,f_jumpingtrap));
                    break;
                case f_zombie:
                    m_bmap[AnaMap.ArrMap.get(i).GetX()][AnaMap.ArrMap.get(i).GetY()] = f_zombie;
                    temp = new Unit(AppManager.getInstance().getBitmap(R.drawable.zombie));
                    temp.SetPos(AnaMap.ArrMap.get(i).GetX(), AnaMap.ArrMap.get(i).GetY());
                    temp.resizebitmap(25, 25);
                    PlayerUnit.add(new UnitManager(temp,5,1,f_zombie));
                    break;
            }
           if(AnaMap.ArrMap.get(i).GetPosition()==f_tower) {
