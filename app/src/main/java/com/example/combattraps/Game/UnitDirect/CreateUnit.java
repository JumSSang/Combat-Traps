...
//        Sound.getInstance().play(4);
    ...
    static  public void CreateMagicTower(int i, int j, Unit temp,ArrayList<Unit_Imfor>MyUnits,ArrayList<Unit_Imfor>EnemyUnits, boolean enemy) {
        if (enemy == true) {
...
//                Sound.getInstance().play(1);
    ...
    static public void CreateHall(int i, int j,ArrayList<Unit_Imfor>MyUnits,ArrayList<Unit_Imfor>EnemyUnits, boolean whounit) {
        //  UnitValue.m_map[i][j] = 3;//원 위치
        // UnitValue.m_map[i][j + 1] = 3; //y값 증가
        //UnitValue.m_map[i + 1][j] = 3; //left값 증가 +1
        //UnitValue.m_map[i + 1][j + 1] = 3; //y left값 증가 +1
        ...
        {
            EnemyUnits.add(new Unit_Imfor(temp, 5, 1, UnitValue.F_TOWNHALL,false));
            EnemyUnits.get(0).myUnitObject.addPosition(new Vec2(i, j));
        }
    }

    static public void CreateBoom(int i,int j,ArrayList<Unit_Imfor>MyUnits,boolean whounit)
    {
        if (whounit == true) {
            if (UnitValue.m_map[i][j] != 3 && UnitValue.m_map[i][j + 1] != 3 && UnitValue.m_map[i + 1][j + 1] != 3 && UnitValue.m_map[i + 1][j] != 3) {
//                Sound.getInstance().play(1);
                ...
                temp.SetPos(i,j);
                Unit_Imfor stemp = new Unit_Imfor(temp, 50, 0, UnitValue.F_BOOM,false);
                //  Units.EnemyUnits.add(stemp);
                stemp.setState(UnitValue.S_MOVE);
                //움직이는 유닛 같은 경우에는 이 타일도 같이 움직여 줘야한다.
                stemp.myUnitObject.addPosition(new Vec2(i, j));
